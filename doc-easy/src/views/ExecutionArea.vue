<template>
  <div class="tudo">
    <div class="conteudo">
      <TimeLine :planName="this.$route.params.name"  @update-card="updateCard"></TimeLine>
    </div>
    <div class="area-card">
      <v-card :key="reset" :class="{'central':true, 'inicial': !iniciado}" :title="cardTitle" width="1500" height="800" >
        <v-card-text>
            <span id="conteudo-card">
              {{ cardContent }}
            </span>
        </v-card-text>
...
export default {
  ...
  methods: {
    ...
    mountMarkdownViewer() {
      let string = this.cardContent;

      setTimeout(()=>{
        document.getElementById('conteudo-card').innerHTML = marked.parse(string);
      }, 50);
    },
    async finish() {
      window.links = [];
